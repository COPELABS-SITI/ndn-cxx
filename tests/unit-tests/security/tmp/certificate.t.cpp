/* -*- Mode:C++; c-file-style:"gnu"; indent-tabs-mode:nil; -*- */
/**
 * Copyright (c) 2013-2015 Regents of the University of California.
 *
 * This file is part of ndn-cxx library (NDN C++ library with eXperimental eXtensions).
 *
 * ndn-cxx library is free software: you can redistribute it and/or modify it under the
 * terms of the GNU Lesser General Public License as published by the Free Software
 * Foundation, either version 3 of the License, or (at your option) any later version.
 *
 * ndn-cxx library is distributed in the hope that it will be useful, but WITHOUT ANY
 * WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A
 * PARTICULAR PURPOSE.  See the GNU Lesser General Public License for more details.
 *
 * You should have received copies of the GNU General Public License and GNU Lesser
 * General Public License along with ndn-cxx, e.g., in COPYING.md file.  If not, see
 * <http://www.gnu.org/licenses/>.
 *
 * See AUTHORS.md for complete list of ndn-cxx authors and contributors.
 *
 * @author Zhiyi Zhang <dreamerbarrychang@gmail.com>
 */

#include "security/tmp/certificate.hpp"

#include "boost-test.hpp"

namespace ndn {
namespace security {
namespace tmp {

BOOST_AUTO_TEST_SUITE(SecurityCertificate)

const uint8_t PUBLIC_KEY[] = {

};

const uint8_t SIG_INFO[] = {
  0x16, 0x55, 0x1B, 0x01, 0x01, 0x1C, 0x26, 0x07, 0x24, 0x08, 0x03, 0x6E, 0x64, 0x6E, 0x08, 0x05,
  0x73, 0x69, 0x74, 0x65, 0x31, 0x08, 0x11, 0x6B, 0x73, 0x6B, 0x2D, 0x32, 0x35, 0x31, 0x36, 0x34,
  0x32, 0x35, 0x33, 0x37, 0x37, 0x30, 0x39, 0x34, 0x08, 0x03, 0x4B, 0x45, 0x59, 0xFD, 0x00, 0xFD,
  0x26, 0xFD, 0x00, 0xFE, 0x0F, 0x32, 0x30, 0x31, 0x35, 0x30, 0x38, 0x31, 0x34, 0x54, 0x32, 0x32,
  0x33, 0x37, 0x33, 0x39, 0xFD, 0x00, 0xFF, 0x0F, 0x32, 0x30, 0x31, 0x35, 0x30, 0x38, 0x31, 0x38,
  0x54, 0x32, 0x32, 0x33, 0x37, 0x33, 0x38
};

const uint8_t SIG_VALUE[] = {
  0x17, 0x80, // SignatureValue
    
};

const uint8_t CERT[] = {
  0x06, 0xFD, 0x01, 0xBB, // Data
    0x07, 0x33, // Name /ndn/site1/ksk-1416425377094/KEY/0123/%FD%00%00%01I%C9%8B
      0x08, 0x03, 0x6E, 0x64, 0x6E,
      0x08, 0x05, 0x73, 0x69, 0x74, 0x65, 0x31,
      0x08, 0x11,
        0x6B, 0x73, 0x6B, 0x2D, 0x31, 0x34, 0x31, 0x36, 0x34, 0x32, 0x35, 0x33, 0x37, 0x37, 0x30, 0x39,
        0x34,
      0x08, 0x03, 0x4B, 0x45, 0x59,
      0x08, 0x04, 0x30, 0x31, 0x32, 0x33,
      0x08, 0x07, 0xFD, 0x00, 0x00, 0x01, 0x49, 0xC9, 0x8B,
    0x14, 0x09, // MetaInfo
      0x18, 0x01, 0x02, // ContentType = Key
      0x19, 0x04, 0x00, 0x36, 0xEE, 0x80, // FreshnessPeriod = 3600000 ms
    0x15, 0xA0, // Content
      , // SignatureInfo
      0x1B, 0x01, 0x01, // SignatureType
      0x1C, 0x26, // KeyLocator: /ndn/site1/ksk-2516425377094/KEY
        0x07, 0x24,
          0x08, 0x03, 0x6E, 0x64, 0x6E,
          0x08, 0x05, 0x73, 0x69, 0x74, 0x65, 0x31,
          0x08, 0x11,
            0x6B, 0x73, 0x6B, 0x2D, 0x32, 0x35, 0x31, 0x36, 0x34, 0x32, 0x35, 0x33, 0x37, 0x37, 0x30, 0x39,
            0x34,
          0x08, 0x03, 0x4B, 0x45, 0x59,
      0xFD, 0x00, 0xFD, 0x26,
        0xFD, 0x00, 0xFE, 0x0F,
          0x32, 0x30, 0x31, 0x35, 0x30, 0x38, 0x31, 0x34, 0x54, 0x32, 0x32, 0x33, 0x37, 0x33, 0x39,
        0xFD, 0x00, 0xFF, 0x0F,
          0x32, 0x30, 0x31, 0x35, 0x30, 0x38, 0x31, 0x38, 0x54, 0x32, 0x32, 0x33, 0x37, 0x33, 0x38,
    0x17, 0x80, // SignatureValue
      
};

BOOST_AUTO_TEST_CASE(Construction)
{
  Block block(CERT, sizeof(CERT));
  Certificate certificate1(block);

  BOOST_CHECK_EQUAL(certificate1.getName(), Name("/ndn/site1/ksk-1416425377094/KEY/0123/%FD%00%00%01I%C9%8B"));
  BOOST_CHECK_EQUAL(certificate1.getKeyName(), Name("/ndn/site1/ksk-1416425377094/KEY"));
  BOOST_CHECK_EQUAL(certificate1.getIdentity(), Name("/ndn/site1"));
  BOOST_CHECK_EQUAL(certificate1.getIssuerId(), name::Component("0123"));
  BOOST_CHECK_EQUAL(certificate1.getKeyId(), name::Component("ksk-1416425377094"));
  BOOST_CHECK_EQUAL(certificate1.getIssuerName(), Name("/ndn/site1/ksk-2516425377094/KEY"));

  Certificate certificate2(std::move(Data(block)));

  BOOST_CHECK_EQUAL(certificate2.getName(), Name("/ndn/site1/ksk-1416425377094/KEY/0123/%FD%00%00%01I%C9%8B"));
  BOOST_CHECK_EQUAL(certificate2.getKeyName(), Name("/ndn/site1/ksk-1416425377094/KEY"));
  BOOST_CHECK_EQUAL(certificate2.getIdentity(), Name("/ndn/site1"));
  BOOST_CHECK_EQUAL(certificate2.getIssuerId(), name::Component("0123"));
  BOOST_CHECK_EQUAL(certificate2.getKeyId(), name::Component("ksk-1416425377094"));
  BOOST_CHECK_EQUAL(certificate2.getIssuerName(), Name("/ndn/site1/ksk-2516425377094/KEY"));
}

BOOST_AUTO_TEST_CASE(GetterSetter)
{
  Certificate certificate;
  certificate.setName("/ndn/site1/ksk-1416425377094/KEY/0123/%FD%00%00%01I%C9%8B");
  certificate.setFreshnessPeriod(time::seconds(3600));
  certificate.setContent(PUBLIC_KEY, sizeof(PUBLIC_KEY));

  Block block1(SIG_INFO, sizeof(SIG_INFO));
  SignatureInfo signatureInfo(block1);

  Name keyLocatorName("/ndn/site1/ksk-2516425377094/KEY");
  KeyLocator keyLocator(keyLocatorName);
  signatureInfo.setKeyLocator(keyLocator);

  time::system_clock::TimePoint notBefore = time::fromIsoString("20150819T120000");
  time::system_clock::TimePoint notAfter = time::fromIsoString("20150823T120000");
  ValidityPeriod period(notBefore, notAfter);
  signatureInfo.setValidityPeriod(period);

  Signature signature(signatureInfo);
  Block block2(SIG_VALUE, sizeof(SIG_VALUE));
  signature.setValue(block2);
  certificate.setSignature(signature);

  BOOST_CHECK_EQUAL(certificate.getName(), Name("/ndn/site1/ksk-1416425377094/KEY/0123/%FD%00%00%01I%C9%8B"));
  BOOST_CHECK_EQUAL(certificate.getKeyName(), Name("/ndn/site1/ksk-1416425377094/KEY"));
  BOOST_CHECK_EQUAL(certificate.getIdentity(), Name("/ndn/site1"));
  BOOST_CHECK_EQUAL(certificate.getIssuerId(), name::Component("0123"));
  BOOST_CHECK_EQUAL(certificate.getKeyId(), name::Component("ksk-1416425377094"));
  BOOST_CHECK_EQUAL(certificate.getIssuerName(), Name("/ndn/site1/ksk-2516425377094/KEY"));
}


BOOST_AUTO_TEST_CASE(ValidityPeriodChecking)
{
  Certificate certificate;
  certificate.setName("/ndn/site1/ksk-1416425377094/KEY/0123/%FD%00%00%01I%C9%8B");
  certificate.setFreshnessPeriod(time::seconds(3600));
  certificate.setContent(PUBLIC_KEY, sizeof(PUBLIC_KEY));

  Block block1(SIG_INFO, sizeof(SIG_INFO));
  SignatureInfo signatureInfo(block1);

  Name keyLocatorName("/ndn/site1/ksk-2516425377094/KEY");
  KeyLocator keyLocator(keyLocatorName);
  signatureInfo.setKeyLocator(keyLocator);

  time::system_clock::TimePoint notBefore = time::fromIsoString("20150819T120000");
  time::system_clock::TimePoint notAfter = time::fromIsoString("20150823T120000");
  ValidityPeriod period(notBefore, notAfter);
  signatureInfo.setValidityPeriod(period);

  Signature signature(signatureInfo);
  Block block2(SIG_VALUE, sizeof(SIG_VALUE));
  signature.setValue(block2);
  certificate.setSignature(signature);

  BOOST_CHECK_EQUAL(certificate.isInValidityPeriod(time::fromIsoString("20150819T115959")), false);
  BOOST_CHECK_EQUAL(certificate.isInValidityPeriod(time::fromIsoString("20150819T120000")), true);
  BOOST_CHECK_EQUAL(certificate.isInValidityPeriod(time::fromIsoString("20150823T120000")), true);
  BOOST_CHECK_EQUAL(certificate.isInValidityPeriod(time::fromIsoString("20150823T120001")), false);
  BOOST_CHECK_EQUAL(certificate.isInValidityPeriod(time::fromIsoString("20150921T130000")), false);
}

BOOST_AUTO_TEST_SUITE_END()

} // namespace tmp
} // namespace securtiy
} // namespace ndn
