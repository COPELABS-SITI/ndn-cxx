...
{
  return (m_container.m_pibImpl == other.m_container.m_pibImpl &&
          m_it == other.m_it);
}
...
{
  return const_iterator(m_identityNames.end(), *this);
}
...
IdentityContainer::get(const Name& identityName) const
{
  ...
  if (it != m_identities.end()) {
    ...
    m_identities[identityName] =
      shared_ptr<detail::IdentityImpl>(new detail::IdentityImpl(identityName, m_pibImpl, false));
  }
  return Identity(id);
}
