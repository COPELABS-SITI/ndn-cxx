...
public:
   ...
   *
   * Password should be stored in @p buf and should not be longer than @p size
   * A typical routine will ask the user to verify the passphrase (for example
   * by prompting for it twice) if @p rwflag is 1
   */
  typedef function<int(char* buf, int size, int rwflag)> PasswordCallback;
...
public:
  ...
  /**
   * @brief Load the private key in encrypted PKCS#8 format from a buffer @p buf with passphrase obtained from @p pwCallback
   *
  ...
  /**
   * @brief Load the private key in encrypted PKCS#8 format from a stream @p is with passphrase obtained from @p pwCallback
   *
   * The default password callback is provided by OpenSSL
   */
  void
  ...
  /**
   * @brief Load the private key in base64-encoded encrypted PKCS#8 format from a buffer @p buf with passphrase @p pw
   *
   * @pre strlen(pw) == pwLen
   */
  void
  ...
  /**
   * @brief Load the private key in base64-encoded encrypted PKCS#8 format from a stream @p is with passphrase @p pw
   *
   * @pre strlen(pw) == pwLen
   */
  void
  ...
  /**
   * @brief Load the private key in base64-encoded encrypted PKCS#8 format from a stream @p is with passphrase obtained from @p pwCallback
   *
   * The default password callback is provided by OpenSSL
   */
  void
  ...
  /**
   * @brief Save the private key in encrypted PKCS#8 format into a stream @p os with passphrase obtained from @p pwCallback
   *
   * The default password callback is provided by OpenSSL
   */
  void
  ...
  /**
   * @brief Save the private key in base64-encoded encrypted PKCS#8 format into a stream @p os with passphrase obtained from @p pwCallback
   *
   * The default password callback is provided by OpenSSL
   */
  void
