  ...
  : m_impl(new Impl)
{
  ...
  case BlockCipherAlgorithm::AES_CBC:
    ...
    break;
  default:
    BOOST_THROW_EXCEPTION(Error(getIndex(), "Cipher mode is not supported"));
  }
}
...
BlockCipher::fillOutputBuffer()
{
  ...
  unique_ptr<OBuffer> buffer(new OBuffer(nRead));
                              ...
                              CipherOperator op)
{
  ...
  const EVP_CIPHER* ciperType = nullptr;
  switch (keyLen) {
  case 16:
    ciperType = EVP_aes_128_cbc();
    break;
  case 24:
    ciperType = EVP_aes_192_cbc();
    break;
  case 32:
    ciperType = EVP_aes_256_cbc();
    break;
  default:
    BOOST_THROW_EXCEPTION(Error(getIndex(), "Key length is not supported"));
  }
  BIO_set_cipher(m_impl->m_cipher, ciperType, key, iv, static_cast<int>(op));
}
...
blockCipher(BlockCipherAlgorithm algo, CipherOperator op,
            ...
            const uint8_t* iv, size_t ivLen)
{
  return unique_ptr<Transform>(new BlockCipher(algo, op, key, keyLen, iv, ivLen));
}
