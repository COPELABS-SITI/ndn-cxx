...
public:
  ...
  Impl(const std::string& dir)
  {
    if (dir.empty()) {
      ...
      if (homeDir == nullptr) {
        BOOST_THROW_EXCEPTION(Error("Environment variable HOME is not set"));
      }
      keystorePath = boost::filesystem::path(homeDir) / ".ndn" / "ndnsec-key-file";
    }
    else
      keystorePath = boost::filesystem::path(dir) / ".ndn" / "ndnsec-key-file";
...
BackEndFile::doGetKeyHandle(const Name& keyName) const
{
  ...
  return unique_ptr<KeyHandle>(new KeyHandleMem(loadKey(keyName)));
}
...
BackEndFile::doCreateKey(const Name& identityName, const KeyParams& params)
{
  ...
  unique_ptr<KeyHandle> keyHandle(new KeyHandleMem(key));
